...
class PersistentDB private (val store: FilePageStore) extends DB:
  ...
  private[petradb] def withBatch(fn: WriteBatch => Unit): Unit =
    activeTxn match
      case Some(txn) => fn(txn)
      case None      => store.modify(fn)

  private[petradb] def readPage(id: PageId): Array[Byte] =
    activeTxn match
  ...
  private[petradb] def persistCatalog(): Unit =
    store.modify { batch =>
      writeCatalogInBatch(batch)
    }

  private[petradb] def writeCatalogInBatch(batch: WriteBatch): Unit =
...
class PersistentTable(
  ...
  private[petradb] def freeAllPages(): Unit =
    store.modify { batch =>
      freeAllDataPages(batch)
