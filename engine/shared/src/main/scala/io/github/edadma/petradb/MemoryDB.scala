...
class MemoryDB extends DB:
  ...
  private[petradb] def recordInsert(table: MemoryTable, node: DLListNode[Array[Value]]): Unit =
    currentUndoLog.foreach(_ += UndoInsert(table, node))

  // Called by MemoryTable before a delete
  private[petradb] def recordDelete(table: MemoryTable, data: Array[Value]): Unit =
    currentUndoLog.foreach(_ += UndoDelete(table, data))

  // Called by MemoryTable before an update
  private[petradb] def recordUpdate(table: MemoryTable, node: DLListNode[Array[Value]], oldData: Array[Value]): Unit =
...
class MemoryTable(name: String, specs: Seq[Spec], private[petradb] val db: MemoryDB) extends Table(name, specs):
  ...
  private[petradb] def undoInsert(node: DLListNode[Array[Value]]): Unit =
    val row = node.element
    for (idxName, idx) <- tableIndexes do
      val midx = idx.asInstanceOf[MemoryTableIndex]
      val baseKey = midx.columnIndices.map(i => row(i): Value)
  ...
  private[petradb] def undoDelete(rowData: Array[Value]): Unit =
  ...
  private[petradb] def undoUpdate(node: DLListNode[Array[Value]], oldData: Array[Value]): Unit =
