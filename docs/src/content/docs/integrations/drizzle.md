...
Create the table through the session, or use [migrations](#migrations) with `drizzle-kit generate` + `migrate()`:
...
Apply [Drizzle Kit](https://orm.drizzle.team/docs/kit-overview) migrations with the `migrate()` function:
...
This reads the migration journal and SQL files generated by `drizzle-kit generate`, executes them in order, and tracks applied migrations in a `drizzle.__drizzle_migrations` table (created automatically).
...
await migrate(db, { migrationsFolder: "./drizzle" });
// Tables are now created — use db normally
```
