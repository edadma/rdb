...
PetraDB fonctionne avec tout serveur GraphQL qui s'execute sur Node.js. Ce guide utilise [GraphQL Yoga](https://the-guild.dev/graphql/yoga-server) -- un serveur leger et conforme aux specifications -- mais la meme approche s'applique a Apollo Server, Mercurius ou tout autre framework. PetraDB s'execute en processus, donc les resolvers appellent le moteur directement sans aller-retour reseau.
...
Definissez un schema GraphQL correspondant a vos tables PetraDB :
...
Utilisez toujours `db.prepare()` avec des parametres positionnels (`$1`, `$2`, ...) pour les valeurs fournies par l'utilisateur. Cela previent l'injection SQL et gere la conversion de types automatiquement.
...
Pour les donnees liees, ajoutez des resolvers de champs qui executent des requetes supplementaires :
...
La pagination par decalage correspond directement au SQL `LIMIT` et `OFFSET` :
...
const resolvers = {
  Query: {
    users: async (_, { first = 10, after }) => {
      ...
      params.push(first + 1); // recuperer un element supplementaire pour verifier hasNextPage
...
Encapsulez les mutations multi-etapes dans une transaction avec `BEGIN` / `COMMIT` / `ROLLBACK` :
...
Utilisez les agregats SQL et retournez des champs calcules :
...
| `DATE` / `TIMESTAMP` | `Date` | `String` (ou scalaire personnalise) |
| `JSON` | `object` | scalaire personnalise ou `String` |
